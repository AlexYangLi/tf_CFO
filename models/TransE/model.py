...
class TransE:
    def __init__(self, kg: KnowledgeGraph, embedding_dim, margin_value, score_func, batch_size, learning_rate,
                 n_generator, n_rank_calculator, max_epoch, eval_freq):
        self.kg = kg
    ...
    def launch_training(self, session):
        ...
        print('epoch triple_avg_loss: {:.6f}'.format(epoch_loss / n_used_triple))
            ...
            if idx_predictions is None:
                ...
                for candidate in idx_head_prediction[::-1]:
                    if candidate == head:
                        ...
                        head_rank_raw += 1
                        if candidate in self.kg.golden_triple_pool and \
                                (relation, tail) in self.kg.golden_triple_pool[candidate]:
                            continue
                        else:
                            head_rank_filter += 1
                ...
                for candidate in idx_tail_prediction[::-1]:
                    if candidate == tail:
                        ...
                        tail_rank_raw += 1
                        if head in self.kg.golden_triple_pool and \
                                (relation, candidate) in self.kg.golden_triple_pool:
                            continue
                        else:
                            tail_rank_filter += 1
    ...
    def train(self, session):
        ...
        for epoch in range(self.max_epoch):
            ...
            self.launch_training(session)
            if (epoch + 1) % self.eval_freq == 0:
                self.launch_evaluation(session)

            saver.save(session, save_path=os.path.join(self.model_path, self.model_name), global_step=epoch)
