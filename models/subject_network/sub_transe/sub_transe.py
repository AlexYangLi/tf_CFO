...
class SubTransE(object):
    def __init__(self, config, sess, word_embeddings, subject_embeddings):
        self.n_words = config.n_words
        self.n_subjects = config.n_subjects
        self.word_embed_size = config.word_embed_size
        self.sub_embed_size = config.sub_embed_size
        ...
        self.word_embed_trainable = config.word_embed_trainable  # whether to fine tune word embeddings
        self.sub_embed_trainable = config.sub_embed_trainable    # whether to fina tune subject embeddings

        self.model_path = './save_model'
        if not os.path.exists(self.model_path):
            os.makedirs(self.model_path)
        self.model_name = 'SubTransE'
        ...
        self.pos_sub_ids = tf.placeholder(tf.int32, name='pos_sub_ids')  # positive relations
        self.neg_sub_ids = tf.placeholder(tf.int32, name='neg_sub_ids')  # negative relations
        ...
        self.subject_embeddings = tf.Variable(initial_value=subject_embeddings, name='sub_embeddings', dtype=tf.float32,
                                              trainable=self.sub_embed_trainable)

        self.pos_sub_embed = tf.nn.embedding_lookup(self.subject_embeddings, self.pos_sub_ids)
        self.neg_sub_embed = tf.nn.embedding_lookup(self.subject_embeddings, self.neg_sub_ids)
    ...
    def build_model(self):
        ...
        w_linear = tf.Variable(tf.truncated_normal([2 * self.hidden_size, self.sub_embed_size], stddev=0.01),
                               name='w_linear')
        b_linear = tf.Variable(tf.constant(0.1, shape=[self.sub_embed_size]), name='b_linear')
        question_embed = tf.matmul(question_gru, w_linear) + b_linear

        # similarity layer
        self.pos_similarity = self.cosine_sim(question_embed, self.pos_sub_embed, name='pos_similarity')
        self.neg_similarity = self.cosine_sim(question_embed, self.neg_sub_embed, name='neg_similarity')
    ...
    def train(self, train_data, valid_data, valid_data_metric):
        ...
        question_ids, sequence_len, pos_sub_ids, neg_sub_ids = train_data
        ...
        for epoch in range(self.n_epoch):
            ...
            for i in range(loops):
                feed_dict = {self.question_ids: question_ids[shuffle_index[i: i + self.batch_size]],
                             ...
                             self.pos_sub_ids: pos_sub_ids[shuffle_index[i: i + self.batch_size]],
                             self.neg_sub_ids: neg_sub_ids[shuffle_index[i: i + self.batch_size]],
    ...
    def valid(self, valid_data):
        question_ids, sequence_len, pos_sub_ids, neg_sub_ids = valid_data
        ...
        feed_dict = {self.question_ids: question_ids[:25600],
                     ...
                     self.pos_sub_ids: pos_sub_ids[:25600],
                     self.neg_sub_ids: neg_sub_ids[:25600],
    ...
    def compute_accuracy(self, data_metric, data_type):
        ...
        q_word_ids, q_seq_len, q_pos_sub_ids, cand_sub_ids = data_metric
        top1 = top3 = top5 = top10 = 0

        data_size = min(len(q_word_ids), 100)  # compute accuracy cost time
        for i in range(data_size):
            score = {}

            # compute score for each candidate relation
            q_word_ids_npy = np.zeros([1, self.max_sequence_len])
            q_word_ids_npy[0, :len(q_word_ids[i])] = q_word_ids[i]
            mul_q_word_ids_npy = np.tile(q_word_ids_npy, (len(cand_sub_ids[i]), 1))

            mul_q_seq_len = np.tile(q_seq_len[i:i + 1], len(cand_sub_ids[i]))
